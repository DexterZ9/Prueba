...
sendAiri: {
    async value(jid, text = '', title, body, largerThumbnail, thumbnail, sourceUrl, quoted) {
        try {
            let thumbBuffer = thumbnail;
            if (typeof thumbnail === 'string' && thumbnail.startsWith('http')) {
                const response = await fetch(thumbnail);
                if (!response.ok) throw new Error(`Error al obtener la miniatura: ${response.statusText}`);
            ...
            console.error('Error en sendAiri:', error.message);
...
sendAiri2: {
    async value(jid, title, body, text = '', largerThumbnail, thumbnail, sourceUrl, quoted) {
        try {
            let thumbBuffer = thumbnail;

            // Si el thumbnail es una URL, descargar y convertirla en un buffer
            if (typeof thumbnail === 'string' && thumbnail.startsWith('http')) {
                const response = await fetch(thumbnail);
            ...
            console.error('Error in sendAiri:', error.message);
            throw new Error('No se pudo enviar el mensaje.');
        }
    },
    enumerable: true,
